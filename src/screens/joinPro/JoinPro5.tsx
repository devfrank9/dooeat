...
import {UnColoredBtn} from '../../styles/BtnStyled';
  ...
  return (
    <BaseScreen>
      <AlignBase>
        ...
        <NextBtn onClick={() => navigate('/join-pro/6')}>건너뛰기</NextBtn>
        <ValidBtn
          onClick={() => {
            if (inputUrl.length !== 0) return;
            else processSignup();
          }}
          status={inputUrl.length === 0 ? 'err' : 'succ'}
        >
          다음
        </ValidBtn>
      </AlignBase>
...
const NextBtn = styled(UnColoredBtn)`
  position: absolute;
  margin: 0 auto;
  left: 50%;
