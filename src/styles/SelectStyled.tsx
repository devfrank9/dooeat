...
interface Prop {
  items?: {id: number; region: {key: number; name: string; state: string[]}}[];
  value?: string;
      ...
      <DropdownContainer>
        ...
        <DropdownMenu isActive={isActive}>
          {example.map(item => (
            ...
            >
              <ItemName id="item_name">{item.region.name}</ItemName>
            </DropdownItemContainer>
          ))}
        </DropdownMenu>
      </DropdownContainer>
      ...
      <DropdownContainer>
        ...
        <DropdownMenu isActive={isActive2}>
          {example.map((item, index) => (
            <DropdownItemContainer
              id="item2"
              key={item.region.state[index]}
              onClick={onSelectItem2}
            >
              <ItemName id="item_name2">{item.region.state[index]}</ItemName>
            </DropdownItemContainer>
          ))}
        </DropdownMenu>
      </DropdownContainer>
